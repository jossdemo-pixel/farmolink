...
const GREETING = 'Ola! Sou o FarmoBot. Como posso ajudar?';
...
export const ChatBot: React.FC = () => {
  ...
  const loadSessionHistory = async (userId: string, sessionId: string) => {
    const history = await fetchChatHistory(userId, sessionId);
    if (history.length > 0) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setUser(data.user);
      const loadedSessions = await fetchChatSessions(data.user.id);

      if (loadedSessions.length > 0) {
        setSessions(loadedSessions);
        setActiveSessionId(loadedSessions[0].id);
        await loadSessionHistory(data.user.id, loadedSessions[0].id);
        return;
      }

      const created = await createChatSession(data.user.id, 'Nova conversa');
      if (created) {
        setSessions([created]);
      ...
      const legacyHistory = await fetchChatHistory(data.user.id);
      if (legacyHistory.length > 0) {
        setMessages(legacyHistory.map((h: any) => ({ role: h.role, text: h.content })));
      } else {
        setMessages([{ role: 'model', text: GREETING }]);
      }
    };
  ...
  const handleNewSession = async () => {
    if (!user?.id) return;
    const created = await createChatSession(user.id, 'Nova conversa');
    if (!created) return;
  ...
  const handleDeleteSession = async () => {
    ...
    const ok = await deleteChatSession(user.id, activeSessionId);
    if (!ok) return;
    ...
    if (remaining.length > 0) {
      ...
      await loadSessionHistory(user.id, remaining[0].id);
      return;
    }
        ...
        <div className="mb-4 w-[350px] sm:w-[380px] h-[500px] bg-white rounded-[32px] shadow-2xl border border-emerald-100 flex flex-col overflow-hidden animate-scale-in">
          <div className="bg-emerald-600 p-5 text-white flex justify-between items-center shrink-0 shadow-lg">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleNewSession} className="hover:bg-white/10 p-2 rounded-full" title="Nova conversa"><Plus size={16} /></button>
              <button onClick={handleDeleteSession} className="hover:bg-white/10 p-2 rounded-full" title="Apagar conversa"><Trash2 size={16} /></button>
          ...
          {sessions.length > 0 && (
            <div className="px-4 py-2 border-b bg-white">
              <select
                className="w-full text-xs p-2 rounded-lg border border-emerald-100 outline-none"
                value={activeSessionId || ''}
                onChange={async (e) => {
                  const nextId = e.target.value;
                  setActiveSessionId(nextId);
                  if (user?.id) await loadSessionHistory(user.id, nextId);
                }}
              >
                {sessions.map((session) => (
                  <option key={session.id} value={session.id}>{session.title || 'Conversa'}</option>
                ))}
              </select>
            </div>
          )}
